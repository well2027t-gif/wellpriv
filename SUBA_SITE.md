# 🚀 Checklist - Suba Suba Page

## Status do Deploy do GitHub Pages

### Fase 1: Preparação do Repositório
- [x] Projeto limpo e otimizado
- [x] README.md criado com instruções
- [x] Código testado e funcionando localmente
- [x] Todos os arquivos necessários no repositório

### Fase 2: Configuração do GitHub Pages
- [ ] Acessar Settings do repositório no GitHub
- [ ] Ir em "Pages" (Páginas)
- [ ] Selecionar "Deploy from a branch"
- [ ] Escolher branch: `main`
- [ ] Escolher pasta: `/root` (ou `/docs` se preferir)
- [ ] Salvar configurações
- [ ] Aguardar o primeiro deploy automático

### Fase 3: Build e Deploy Manual (Alternativa)
- [ ] Executar `pnpm install` localmente
- [ ] Executar `pnpm build` para gerar arquivos em `dist/`
- [ ] Criar branch `gh-pages` (se não existir)
- [ ] Fazer push dos arquivos de `dist/` para a branch `gh-pages`
- [ ] Configurar GitHub Pages para usar a branch `gh-pages`

### Fase 4: Validação e Publicação
- [ ] Acessar o site em `https://well2027t-gif.github.io/wellpriv`
- [ ] Verificar se todos os links funcionam
- [ ] Testar responsividade em mobile
- [ ] Confirmar que as animações carregam corretamente
- [ ] Publicar link final

### Fase 5: Configuração de Domínio Customizado (Opcional)
- [ ] Adquirir domínio (ex: wellpriv.link)
- [ ] Configurar DNS do domínio
- [ ] Adicionar CNAME no GitHub Pages
- [ ] Ativar HTTPS automático

---

## 📋 Próximos Passos

1. **Ir para o GitHub:** https://github.com/well2027t-gif/wellpriv
2. **Acessar Settings** → **Pages**
3. **Configurar conforme as instruções acima**
4. **Aguardar o deploy** (geralmente leva 1-2 minutos)
5. **Acessar o site publicado**

---

## 🔗 Links Importantes

- **Repositório:** https://github.com/well2027t-gif/wellpriv
- **Site Publicado:** https://well2027t-gif.github.io/wellpriv
- **Documentação GitHub Pages:** https://docs.github.com/en/pages

---

**Última atualização:** 11 de Abril de 2026
